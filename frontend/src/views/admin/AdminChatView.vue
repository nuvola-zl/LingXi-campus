...
async function loadSessions() {
  sessions.value = await chatAPI.getChatHistory('ADMIN')
}
...
async function loadModels() {
  try {
    ...
    if (models.length > 0) {
      ...
      console.warn('[AdminChat] models list is empty')
    }
  } catch (e) {
    console.error('[AdminChat] loadModels error:', e)
  }
}
...
async function newChat() { currentSessionId.value = null; messages.value = [] }

async function sendMessage(data: { text: string; file: File | null }) {
  if (isStreaming.value) return
  const formData = new FormData()
  formData.append('domain', 'ADMIN')
  ...
  try {
    for await (const event of chatAPI.sendMessage(formData, currentSessionId.value, (newId) => {
      ...
      if (event.type === 'ai_prompt') { /* ignore in admin */ }
...
<template>
  <div style="display:flex; flex:1; height:100%; overflow:hidden">
    ...
    <div class="chat-main">
      <div class="domain-bar"><span class="tag">后勤服务</span><select v-model="selectedModelId" class="model-select"><option v-for="m in availableModels" :key="m.id" :value="m.id">{{ m.name }}</option></select><label class="thinking-toggle"><input type="checkbox" v-model="enableThinking" /> 深度思考</label></div>
      <div class="messages" ref="messagesRef">
        <div v-if="messages.length === 0" class="welcome"><h2>灵犀后勤助手</h2><p>场地预约、器材借用、进度查询，一句话办理</p></div>
        <ChatMessage v-for="(m, i) in messages" :key="i" :message="m"
                     :is-stream="isStreaming && i === messages.length - 1" />
      </div>
      <ChatInput :isStreaming="isStreaming" placeholder="如：借个相机 / 明天下午 2 点有什么场地..." @send="sendMessage" @stop="stopStreaming" />
    </div>
  </div>
</template>
