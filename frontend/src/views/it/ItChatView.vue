...
async function loadSessions() {
  sessions.value = await chatAPI.getChatHistory('IT')
}
...
async function loadModels() {
  try {
    ...
    if (models.length > 0) {
      ...
      console.warn('[ItChat] models list is empty')
    }
  } catch (e) {
    console.error('[ItChat] loadModels error:', e)
  }
}
...
<template>
  <div style="display:flex; flex:1; height:100%; overflow:hidden">
    ...
    <div class="chat-main">
      <div class="model-bar">
        <span class="domain-tag">智能报修</span>
        <select v-model="selectedModelId" class="model-select">
          <option v-for="m in availableModels" :key="m.id" :value="m.id">{{ m.name }}</option>
        </select>
        <label class="thinking-toggle"><input type="checkbox" v-model="enableThinking" /> 深度思考</label>
      </div>
      <div class="messages" ref="messagesRef">
        <div v-if="messages.length === 0" class="welcome">
          <h2>灵犀报修助手</h2>
          <p>宿舍/教室设施故障，我会先查校园指南帮您排查，解决不了的一键建单</p>
        </div>
      ...
      <ChatInput ref="chatInputRef" :isStreaming="isStreaming"
                 placeholder="描述您遇到的设施问题，如：宿舍热水器不出热水..." @send="sendMessage" @stop="stopStreaming" />
    </div>
  </div>
</template>
...
.model-bar {
  padding: 0.625rem 1.5rem; border-bottom: 1px solid var(--border-color);
  display: flex; align-items: center; gap: 0.75rem;
  .domain-tag {
    padding: 0.125rem 0.5rem; background: rgba(99,102,241,0.1); color: var(--brand-from);
    border-radius: 0.25rem; font-size: 0.75rem; font-weight: 500;
  }
  .model-select { font-size: 0.8125rem; color: #555; background: transparent; border: 1px solid var(--border-color); border-radius: 0.375rem; padding: 0.25rem 0.5rem; cursor: pointer; outline: none; &:focus { border-color: var(--brand-from); } }
  .thinking-toggle { display: flex; align-items: center; gap: 0.25rem; font-size: 0.8125rem; color: #888; cursor: pointer; user-select: none; input[type="checkbox"] { width: 0.875rem; height: 0.875rem; cursor: pointer; accent-color: var(--brand-from); } }
}
.messages {
